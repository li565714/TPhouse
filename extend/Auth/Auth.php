...
class Auth
{
    ...
    public function getGroups($uid)
    {
        ...
        $user_groups = Db::view($auth_group_access, 'uid,group_id')
                ->view($auth_group, 'id,title,rules', "{$auth_group_access}.group_id={$auth_group}.id", 'LEFT')
    ...
    public function getRuleList($uid)
    {
        ...
        //读取用户组所有权限规则
        $this->rules = Db::name($this->config['auth_rule'])->where($where)->field('id,pid,actions ,icon,url,title')->select();
        //循环规则，判断结果。
        $rulelist = []; //
        if (in_array('*', $ids))
        {
        ...
        {
            //超级管理员无需验证condition
            if (!empty($rule['actions']) && !in_array('*', $ids))
            {
                //根据condition进行验证
                $user = $this->getUserInfo($uid); //获取用户信息,一维数组
                $command = preg_replace('/\{(\w*?)\}/', '$user[\'\\1\']', $rule['actions']);
