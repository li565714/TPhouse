...
strVarCity += "                          <div class=\"data-result\"><em>已选择: <strong><\/strong> <\/em><\/div>";
...
var datatype = "";
var domVal = null;
function appendCity(thiscon, Cityxz,domVals) {
       ...
       // strVarCity += "                              <p class=\"data-title\">全部省份<\/p>";
        ...
        $('.data-result span').each(function(index) {
            ...
            if ($('a[data-code=' + $(this).data("code") + ']').length > 0) {
                ...
                var numlabel = $('a[data-code=' + $(this).data("parentcode") + ']').find('label').text();
//				alert($(this).data("parentcode"));
                $('a[data-code=' + $(this).data("parentcode") + ']').find('label').text(parseInt(numlabel) + 1).show();
            }
        });
    }
    //显示下一级
        ...
        if (typeof (relations[$(con).data("code")]) != "undefined") {
            ...
            for (var i in relations[$(con).data("code")]) {
                strVarCity += '<li><a href="javascript:;" data-parentcode=\"'+$(con).data("code")+'\" class="d-item" data-code="' + relations[$(con).data("code")][i] + '"  data-name=\"' + dataarrary[relations[$(con).data("code")][i]][0] + '\" onclick="selectProvince(\'sub\',this,\'\')">' + dataarrary[relations[$(con).data("code")][i]][0] + '<label>0</label></a></li>';
            }
            strVarCity += "<\/ul>";
            $('.data-container-city').html(strVarCity);
        } else {
            if (datatype == "duoxuan") {
                ...
                //if ($('.data-result span').length > 2) { 最多选择数量
				if (false) {
                    $('.data-error').slideDown();
                    setTimeout("$('.data-error').hide()", 1000);
                    return false;
                } else {
                    $('.data-result').append("<span class=\"svae_box aui-titlespan\"  data-parentcode=\""+$(con).data("parentcode")+"\" data-code=\"" + $(con).data("code") + "\" data-name=\"" + $(con).data("name") + "\" onclick=\"removespan_area(this)\">" + $(con).data("name") + "<i>×<\/i><\/span>");
